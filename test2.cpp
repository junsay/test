...
int main() 
{
	...
	while (window.isOpen()) 
	{
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::W)) // CODE FOR JUMPING
		{
			wReleased = false;
		...
		if ((sf::Keyboard::isKeyPressed(sf::Keyboard::A) && grounded && !crouching)) // if the player is not crouching and grounded.. move Left
		{
			cout << "left" << endl;
			player_accel_h = -properties["H_ACCEL"];
		}
		
		else if ((sf::Keyboard::isKeyPressed(sf::Keyboard::D) && grounded && !crouching)) // if the player is not crouching and grounded.. move Right
		{
			cout << "right" << endl;
			player_accel_h = properties["H_ACCEL"];
		}	
		else if ((sf::Keyboard::isKeyPressed(sf::Keyboard::A) && wReleased && crouching)) // when A was pressed upon launch
		{
			cout << "jump left" << endl;
			player_accel_h = -50;
		}
		else if ((sf::Keyboard::isKeyPressed(sf::Keyboard::D) && wReleased && crouching)) // when D was pressed upon launch
		{
		...
		if (grounded && !crouching) // if on ground and not charging
		{
			//cout << "one" << player_vel_h << endl;
			player_vel_h += baseAccel;
		}
			
		else if (!grounded && !crouching) // if mid air and not charging
		{
			//cout << "two" << player_vel_h << endl;
			player_vel_h += baseAccel;
		}
			
		else if (wReleased && crouching) // upon launch // ISSUE SO FAR, THE MOMENT YOU RELEASE THE A OR D BUTTON, ACCEL BECOMES 0.. Once siguro na wala ng input, accel becomes 0 (else statement above)
		{
			//cout << "three" << player_vel_h << endl;
			player_vel_h += baseAccel;
		}
		else
		{
			//cout << "four" << player_vel_h << endl;
			player_vel_h = 0;
		}
		...
		// continue charging jump if (1) there is jump input, (2) the player is grounded or has not exceeded the V_SAFE limit, and (3) the jump input has not yet been released
		// if (sf::Keyboard::isKeyPressed(sf::Keyboard::W) && ((grounded || safe_counter < properties["V_SAFE"])) && wReleased == false && !(hold_counter == properties["V_HOLD"])) // chargwe while hold_counter does not reach v hold
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::W) && ((grounded || safe_counter < properties["V_SAFE"])) && wReleased == false)
		{
			crouching = true; // set to true while the player is charging
			// if hold counter already exceeds the v hold...
			if (hold_counter >= properties["V_HOLD"]) 
			{
				// make it equal to v hold
				hold_counter = properties["V_HOLD"];
				//cout << hold_counter << " " << properties["V_HOLD"] << endl;
				//cout << "chargeCurrent" << " " << chargeCurrent << endl;
			}
			// else, just increment hold counter and also the current charge (chargeCurrent)
			else  
			{
				chargeCurrent += player_accel_v / FPS; 
				hold_counter++;
				//cout << hold_counter << " " << properties["V_HOLD"] << endl;
				//cout << "chargeCurrent" << " " << chargeCurrent << endl;
			}
		}
		// else if (wReleased|| hold_counter == properties["V_HOLD"]) // if the player releases the key or when hold_counter reaches v hold... jump
		// if the player lets go of the jump input and is still crouching.. jump
		else if (wReleased && crouching)
		{
			wReleased = false;
			crouching = false;
			// cout << wReleased << endl;
			player_vel_v += chargeCurrent;
			
			chargeCurrent = 0;
			hold_counter = 0;
			//cout << player_vel_v << endl;
		}
		else
		{
			baseAccel = properties["GRAVITY"] / FPS; // apply gravity instead if there is no jump input
			player_vel_v += baseAccel;
		}
		...
		{ 
			safe_counter = 0;
			// hold_counter = 0; // do not reset first
		}
		else
